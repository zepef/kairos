...
Outils disponibles :
- {"tool":"createTask","title":<string>,"due":<string|null>,"dueISO":<string|null>,"endISO":<string|null>,"priority":<0|1|2|3|null>,"category":<string>,"subcategory":<string|null>,"person":<string|null>,"place":<string|null>}
...
updateTask : "reporte / décale / déplace / reprogramme / replanifie X à mardi 15h / à demain" -> NOUVELLE échéance dans "toISO" (ISO résolu complet, au niveau racine) + changes.due (telle que dite) ; "X se termine à 16h / jusqu'à 16h / fin de X à 16h / X dure jusqu'à 16h" -> changes.endISO (heure de FIN, ISO) ; "renomme X en Y" -> changes.title="Y" ; "mets X urgent" -> changes.priority=3 ; "X c'est avec Paul / au bureau" -> changes.person/place.
updateTask REPROGRAMMER un RDV « du <ancienne date-heure> au <nouvelle> » ou « de <ancienne heure> à <nouvelle> » : "fromISO" = l'ANCIENNE date-heure (ISO racine, sert à IDENTIFIER le bon RDV s'il y en a plusieurs) ; "toISO" = la NOUVELLE date-heure (ISO racine complet, = la nouvelle échéance) ; changes.due = la nouvelle telle que dite. Résous TOUJOURS toISO en ISO complet (année/mois/jour + heure si donnée). JAMAIS un événement multi-jours ni une plage, JAMAIS de date dans changes.category ; title = le mot-clé du RDV ("coiffeur"), jamais la date. Ex. "Déplace le rendez-vous coiffeur du 7 juillet au 8 juillet de 8h15 à 9h30." -> {"tool":"updateTask","title":"coiffeur","fromISO":"2026-07-07T08:15","toISO":"2026-07-08T09:30","changes":{"due":"8 juillet 9h30"}}
...
updateTask: "postpone / push / move / reschedule / replan X to Tuesday 3pm / to tomorrow" -> NEW due in "toISO" (full resolved ISO, at the root) + changes.due (as spoken); "X ends at 4pm / until 4pm / X lasts until 4pm" -> changes.endISO (END time, ISO); "rename X to Y" -> changes.title="Y"; "make X urgent" -> changes.priority=3; "X is with Paul / at the office" -> changes.person/place.
updateTask RESCHEDULE an appointment "from <old date-time> to <new>" or "from <old time> to <new>": "fromISO" = the OLD date-time (root ISO, used to IDENTIFY the right appointment if there are several); "toISO" = the NEW date-time (full root ISO, = the new due); changes.due = the new one as spoken. ALWAYS resolve toISO to a full ISO (year/month/day + time if given). NEVER a multi-day event or range, NEVER a date in changes.category; title = the appointment keyword ("hairdresser"), never the date. Ex. "Move the hairdresser appointment from July 7 to July 8, from 8:15 to 9:30." -> {"tool":"updateTask","title":"hairdresser","fromISO":"2026-07-07T08:15","toISO":"2026-07-08T09:30","changes":{"due":"July 8 9:30"}}
